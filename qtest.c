...
void kernel_sort(struct list_head *head, bool descend)
{
...
static bool do_shuffle(int argc, char *argv[])
{
    ...
    for (int i = q_size(head) - 1; i >= 0; i--) {  // reduce search time
        int random = rand() % (i + 1);             // 0~i
...
static void console_init()
{
    ...
    ADD_COMMAND(kernel_sort, "Sort queue with list_sort.c", "");
    ...
    ADD_COMMAND(reverseK, "Reverse the nodes of the queue 'K' at a time",
                "[K]");
    ADD_COMMAND(shuffle, "Shuffle the current list", "");
